...
const LoginU = () => {
  ...
  const onSub = async (e) => {
    e.preventDefault();
    setLoading(true);
    const res = await axios.post(`${apilink}/api/login`, {
      email,
      password,
    });

    if (res.data.success) {
      localStorage.setItem('_book_access_user_login', true);

      Cookies.set('_book_access_user_tokon_', res.data.access_token, {
        expires: 1,
      });
